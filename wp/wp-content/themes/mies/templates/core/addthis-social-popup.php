...
if (!empty($share_buttons_types) || $share_buttons_types !== 'false') {
	...
	$display_share_buttons .= '<div class="overlay__wrapper">' . PHP_EOL;
	$share_label = wpgrade::option( 'project_menu_share_label' );
	if ( empty( $share_label ) ) {
		$share_label = __( 'Share', 'mies_txtd' );
	}
	$display_share_buttons .= '<h2 class="share-title  js-share-destination">' . $share_label .'</h2>' . PHP_EOL;
	...
	$display_share_buttons .= '</ul><br/>' . PHP_EOL;
	$display_share_buttons .= '<button class="btn  btn--close-overlay  icon  icon-close"></button>' . PHP_EOL;
